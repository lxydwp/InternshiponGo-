                            ...
                            <div class="col-sm-12 col-md-12 col-lg-12">
                              <div class="card text-dark bg-light">
                                <div class="card-body">
                                  ...
                                  <?php 
                                        $number=$_GET['number'];
                                        $student=mysqli_query($con,"SELECT * from student where contactnumber = '$number'")or die(mysqli_error($con));
                                        $rowstudent=mysqli_fetch_object($student);

                                        
                                if (!empty($rowstudent)) {
                                        $securityQA=mysqli_query($con,"SELECT * from securityquestions where securityquestionsUSERID = '$rowstudent->StudentID' and securityquestionsUSERTYPE = 'student'")or die(mysqli_error($con));
                                        $rowsecurityQA=mysqli_fetch_object($securityQA);
                              

                                        if (empty($rowsecurityQA)) {
                                  ?>
                                        <div class="alert alert-outline-danger" role="alert">You didnt have security questions to reset your password please contact your adviser!</div>
